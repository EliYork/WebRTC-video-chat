...
- 房间页会为用户生成临时昵称，例如 `Guest-1234`。
- 临时昵称保存在 `localStorage`，也可以在房间页直接修改。
- 聊天使用现有 Socket.IO 连接，事件为 `chat:join`、`chat:history`、`chat:send` 和 `chat:message`。
- 每条消息包含 `id`、`roomId`、`senderName`、`content` 和 `createdAt`。
- 服务端在内存中按频道保存最近 50 条消息，服务重启后消息会丢失。
- 消息只广播到同一个频道，例如 `/room/project` 的消息不会出现在 `/room/game`。
- 客户端使用文本节点渲染消息内容，不使用 `innerHTML` 渲染用户输入。

## 光标逻辑

- 共享光标使用现有 Socket.IO 连接，事件为 `cursor:move`、`cursor:leave` 和 `cursor:remove`。
- 光标在整个频道页面内启用，即使没有加入语音或共享屏幕也可以看到同频道用户的鼠标。
- 客户端发送视口相对坐标，`x = event.clientX / window.innerWidth`，`y = event.clientY / window.innerHeight`，不发送真实屏幕像素。
- 服务端只把光标事件广播到同一个频道的其他用户。
- 每个用户的光标颜色基于 Socket.IO `socket.id` 生成，刷新后可能变化。
- 鼠标停止移动或离开页面后，远端光标保留在最后位置并变为半透明。
- 用户断开、刷新或离开频道时，其他客户端会移除该用户光标。
- 第一版只做光标，不做白板、画线或标注。
